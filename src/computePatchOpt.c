...
int computePatchOpt_it(int n, int m)
{
	...
	for(j = 0; j <=m; j++) {
		...
		toPrint = (char*)calloc(1+1+sizeof(int)+1+(lBlen+1)+1,sizeof(char));
		rewind(fA);
		iD=0;
		for(i = 0; i <=n; i++) {
			if(i!=0){
			...
			if (i==0) {
				...
				//i=0 et j!=0  -> f(0,j) = sum(10+LkB) avec k=1..j, calculé à chaque getline
					...
					mem[0][j].cout = L; 
					mem[0][j].cmd = (char*)calloc(ecrit,sizeof(char));
			...
			// i!=0 
				...
				if (j==0) {
					...
					mem[i][0].cmd = (char*)calloc(ecrit,sizeof(char));
				...
				// i!=0 et j!=0
					...
					mem[i][j].cout = min;
					mem[i][j].cmd = (char*)calloc(ecrit,sizeof(char));
